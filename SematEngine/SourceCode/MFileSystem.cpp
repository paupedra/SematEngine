...
bool MFileSystem::Init() //Config& config
{
...
void MFileSystem::CreateLibraryDirectories()
{
...
bool MFileSystem::AddPath(const char* path_or_zip)
{
	bool ret = false;
...
bool MFileSystem::Exists(const char* file) const
{
	return PHYSFS_exists(file) != 0;
}

bool MFileSystem::CreateDir(const char* dir)
{
...
bool MFileSystem::IsDirectory(const char* file) const
{
	return PHYSFS_isDirectory(file) != 0;
}

const char* MFileSystem::GetWriteDir() const
{
	//TODO: erase first annoying dot (".")
	return PHYSFS_getWriteDir();
}

void MFileSystem::DiscoverFiles(const char* directory, std::vector<std::string>& file_list, std::vector<std::string>& dir_list) const
{
...
void MFileSystem::GetAllFilesWithExtension(const char* directory, const char* extension, std::vector<std::string>& file_list) const
{
...
PathNode MFileSystem::GetAllFiles(const char* directory, std::vector<std::string>* filter_ext, std::vector<std::string>* ignore_ext) const
{
	PathNode root;
	if (Exists(directory))
	{
...
void MFileSystem::GetRealDir(const char* path, std::string& output) const
{
...
std::string MFileSystem::GetPathRelativeToAssets(const char* originalPath) const
{
	std::string ret;
...
bool MFileSystem::HasExtension(const char* path) const
{
	std::string ext = "";
...
bool MFileSystem::HasExtension(const char* path, std::string extension) const
{
	std::string ext = "";
	SplitFilePath(path, nullptr, nullptr, &ext);
...
bool MFileSystem::HasExtension(const char* path, std::vector<std::string> extensions) const
{
	std::string ext = "";
	SplitFilePath(path, nullptr, nullptr, &ext);
	if (ext == "")
...
std::string MFileSystem::NormalizePath(const char* full_path) const
{
...
void MFileSystem::SplitFilePath(const char* full_path, std::string* path, std::string* file, std::string* extension) const
{
...
unsigned int MFileSystem::Load(const char* path, const char* file, char** buffer) const
{
...
uint MFileSystem::Load(const char* file, char** buffer) const
{
	uint ret = 0;
...
bool MFileSystem::DuplicateFile(const char* file, const char* dstFolder, std::string& relativePath)
{
...
bool MFileSystem::DuplicateFile(const char* srcFile, const char* dstFile)
{
...
uint MFileSystem::Save(const char* file, const void* buffer, unsigned int size, bool append) const
{
	unsigned int ret = 0;
...
bool MFileSystem::Remove(const char* file)
{
	bool ret = false;
...
uint64 MFileSystem::GetLastModTime(const char* filename)
{
	return PHYSFS_getLastModTime(filename);
}

std::string MFileSystem::GetUniqueName(const char* path, const char* name) const
{
